...
CREATE TABLE Municipality(
   munic_id INT IDENTITY(1, 1) NOT NULL,
   manager_id INT,
   name VARCHAR(255) NOT NULL UNIQUE,
...
CREATE TABLE Citation_Type(
   citation_type_id INT IDENTITY(1, 1) NOT NULL,
   name VARCHAR(255) NOT NULL UNIQUE,
   fine DECIMAL(7,2) NOT NULL,
   training_eligable BIT DEFAULT 0,
...
CREATE TABLE Wanted(
   wanted_id INT NOT NULL IDENTITY(1, 1),
   reference_no VARCHAR(32) NOT NULL UNIQUE,
   dangerous BIT NOT NULL DEFAULT 1,
